...
class VisibleNeuronsHandler:
    ...
    @staticmethod
    def _create_visible_neurons_mask(indices: Dict[str, List[int]]) -> Dict[str, torch.Tensor]:
        """
    ...
    def _load_visible_neuron_indices(self) -> Dict[str, torch.Tensor]:
        ...
        with open(
            ...
            return VisibleNeuronsHandler._create_visible_neurons_mask(pickle.load(pickle_file))
    
    @staticmethod
    def _expand_mask_to_match_last_dim(mask_1d: torch.Tensor, reference_tensor: torch.Tensor):
        """
    ...
    def _expand_masks_to_responses(self, responses_tensor: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        ...
        """
        return {layer: VisibleNeuronsHandler._expand_mask_to_match_last_dim(self.visible_neurons_mask_1d[layer], responses_tensor[layer])
                for layer in responses_tensor}

    def _select_on_mask(self, data_dict: Dict[str, torch.Tensor], mask: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
    ...
    def assign_teacher_forced_responses(self, target_responses: Dict[str, torch.Tensor], prediction_responses: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        ...
        expanded_mask = {layer: expanded_mask[layer].to(nn_model.globals.DEVICE) for layer in expanded_mask}
        ...
        return {layer: torch.where(expanded_mask[layer], target_responses[layer], prediction_responses[layer].detach()) for layer in target_responses}
    
    
    def split_visible_invisible_neurons(self, neuron_responses: Dict[str, torch.Tensor]) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        """
        ...
        visible_neurons = self._select_on_mask(neuron_responses, self.visible_neurons_mask_1d)
        invisible_neurons = self._select_on_mask(neuron_responses, {layer: ~mask for layer, mask in self.visible_neurons_mask_1d.items()})
